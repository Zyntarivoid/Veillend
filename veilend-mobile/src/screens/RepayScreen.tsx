...
export default function RepayScreen() {
  ...
  const validation = validateProtocolAmount(amount, {
    ...
    maxLabel: selectedLoan
      ? formatAmountLimit(selectedLoan.amount, selectedLoan.asset)
      : undefined,
    warningThreshold: selectedLoan?.amount,
    warningMessage: "Full repayment will close this debt.",
  });
  ...
  const confirmRepay = async () => {
    ...
    if (!validation.canSubmit || validation.value == null) {
      Toast.show({
        type: "error",
        text1: "Check amount",
        text2: validation.error ?? "Enter a valid repayment amount.",
      });
      return;
    }
    ...
    try {
      const res = await useStore
        .getState()
        .repay({ amount: submitAmount, asset: selectedLoan.asset });
      Toast.show({
        type: "success",
        text1: "Repay Submitted",
      ...
      const mockRes = {
        ...
        status: "mock",
        amount: submitAmount,
        asset: selectedLoan.asset,
      };
      useStore.setState({ lastLendingTx: mockRes });
      Toast.show({
        type: "info",
        text1: "Offline - Mock Repay",

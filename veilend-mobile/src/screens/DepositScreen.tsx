...
export default function DepositScreen() {
  ...
  const validation = validateProtocolAmount(amount, {
    ...
    maxLabel: selectedAsset
      ? formatAmountLimit(selectedAsset.balance ?? 0, selectedAsset.symbol)
      : undefined,
    warningThreshold:
      selectedAsset?.balance != null ? selectedAsset.balance * 0.9 : undefined,
    warningMessage: "This uses most of your wallet balance.",
  });
  ...
  const confirmDeposit = async () => {
    ...
    if (!validation.canSubmit || validation.value == null) {
      Toast.show({
        type: "error",
        text1: "Check amount",
        text2: validation.error ?? "Enter a valid deposit amount.",
      });
      return;
    }
    ...
    try {
      const res = await useStore
        .getState()
        .deposit({ amount: submitAmount, asset: selectedAsset.symbol });
      Toast.show({
        type: "success",

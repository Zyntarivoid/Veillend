...
import { validateProtocolAmount } from "../utils/amountValidation";
...
export default function BorrowScreen() {
  ...
  const assetPrice = selectedAsset?.price ?? 1;
  const maxBorrowAmount = BORROW_LIMIT_USD / assetPrice;
  const validation = validateProtocolAmount(amount, {
    maxAmount: maxBorrowAmount,
    maxLabel: `$${BORROW_LIMIT_USD.toLocaleString()} borrow limit`,
    warningThreshold: maxBorrowAmount * 0.8,
    warningMessage: "This borrow is close to your limit.",
  });
  ...
  const confirmBorrow = async () => {
    ...
    if (!validation.canSubmit || validation.value == null) {
      Toast.show({
        type: "error",
        text1: "Check amount",
        text2: validation.error ?? "Enter a valid borrow amount.",
      });
      return;
    }
    const submitAmount = String(validation.value);
    try {
      const res = await useStore
        .getState()
        .borrow({ amount: submitAmount, asset: selectedAsset.symbol });
      Toast.show({
        type: "success",
      ...
      Toast.show({
        type: "info",
        text1: "Offline - Mock Borrow",
        text2: JSON.stringify(mockRes),
      });
      setModalVisible(false);
    }
  };
          ...
          {MOCK_ASSETS.map((asset) => (
            ...
            >
              ...
              <View style={styles.assetRight}>
                ...
                <Text style={styles.available}>Available: 10M</Text>
              </View>
            </TouchableOpacity>
          ))}
        </View>
        ...
        >
          <View style={styles.modalContent}>
            ...
            {validation.error ? (
              ...
              <Text style={styles.validationWarning}>{validation.warning}</Text>
            ) : null}
            <View style={styles.modalButtons}>
              ...
              <TouchableOpacity
                ...
                disabled={lendingLoading || !validation.canSubmit}
              >
                {lendingLoading ? (
